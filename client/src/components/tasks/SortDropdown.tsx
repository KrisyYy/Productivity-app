...
export const SortDropdown = ({setTasks}: {setTasks: Dispatch<SetStateAction<ITask[]>>}) => {
    ...
    const handleOnSubmit = (e: any) => {
        ...
        switch(e.target.value) {
            case "name": {
                if (currentSort === "name") {
                    setTasks(state => {console.log(state); state.reverse(); return state.reverse();});
                    break;
                }
        ...
        <div onBlur={handleBlur} className="relative inline-block text-left w-32">
            ...
            <div className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                <div className="py-1">
                    ...
                    <button value="name" onClick={handleOnSubmit} className="text-gray-700 block w-full px-4 py-2 text-left text-sm hover:bg-slate-100">By name</button>
                    <button value="date" onClick={handleOnSubmit} className="text-gray-700 block w-full px-4 py-2 text-left text-sm hover:bg-slate-100">By date</button>
                    </form>
                </div>
            </div>}
        </div>
